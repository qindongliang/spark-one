...
private[authz] final class LogicalPlanResourceExtractor(spark: SparkSession) {
  ...
  private[authz] def managedHdfsAccess(plan: LogicalPlan): Option[ManagedHdfsAccess] = {
    val command = plan.getClass.getName match {
      case "ai.queryone.extension.overwrite.QueryOneManagedHdfsLoadCommand" =>
        Some("workspaceOwner" -> Read)
      case "ai.queryone.extension.overwrite.QueryOneManagedHdfsOverwriteCommand" =>
        Some("tenant" -> Write)
      case _ => None
    }
  ...
  private def logicalRelationResources(
    ...
    relation.catalogTable match {
      ...
      case None => relation.relation match {
        ...
        case baseRelation =>
          queryOneMysqlResources(baseRelation, action) match {
  ...
  private def queryOneMysqlResources(
      relation: org.apache.spark.sql.sources.BaseRelation,
      action: String): Option[Seq[OdepAuthzResource]] = {
    if (relation.getClass.getName != "ai.queryone.provider.mysql.QueryOneMysqlRelation") {
      None
    } else {
      def invoke(name: String): String =
        relation.getClass.getMethod(name).invoke(relation).asInstanceOf[String]
      invoke("queryOneAuthzMode") match {
        case "static" => Some(Seq.empty)
        case "odep" =>
          Some(Seq(OdepAuthzResource.table(
            invoke("queryOneAuthzCatalog"),
            invoke("queryOneAuthzNamespace"),
            invoke("queryOneAuthzTable"),
            action)))
        case mode =>
          throw new OdepAuthorizationException(
            s"Unsupported queryone_mysql authorization mode: $mode")
      }
    }
  }

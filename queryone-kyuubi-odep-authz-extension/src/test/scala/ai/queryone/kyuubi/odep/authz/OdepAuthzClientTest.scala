...
final class OdepAuthzClientTest {
  ...
  def setUp(): Unit = {
    ...
    server.createContext("/api/queryone/authz/check", handle)
    server.start()
  ...
  def usesRuntimePropertiesWhenEnvironmentIsMissing(): Unit = {
    ...
    val properties = Map(
      "queryone.odep.api.url" -> apiUrl,
      "queryone.odep.app.id" -> appId,
      "queryone.odep.sign.key" -> signKey,
      "queryone.odep.connect.timeout.seconds" -> "2",
      "queryone.odep.request.timeout.seconds" -> "2")
  ...
  def prefersEnvironmentOverRuntimeProperties(): Unit = {
    response = allowedJdbcResponse
    val properties = Map(
      "queryone.odep.api.url" -> "ftp://invalid.example",
      "queryone.odep.app.id" -> "wrong-app",
      "queryone.odep.sign.key" -> "wrong-key",
      "queryone.odep.connect.timeout.seconds" -> "invalid",
      "queryone.odep.request.timeout.seconds" -> "invalid")
    val environment = Map(
      "ODEP_API_URL" -> apiUrl,

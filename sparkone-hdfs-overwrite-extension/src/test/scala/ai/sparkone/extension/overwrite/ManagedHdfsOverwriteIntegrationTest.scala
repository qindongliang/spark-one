...
final class ManagedHdfsOverwriteIntegrationTest {
  ...
  def overwritesCompleteResultAndKeepsOldTargetOnWriteFailure(): Unit = {
    val workspace = Files.createTempDirectory("sparkone-overwrite-workspace")
    val warehouse = Files.createTempDirectory("sparkone-overwrite-warehouse")
    ...
    val spark = SparkSession.builder()
      ...
      .config(ManagedHdfsOverwriteExecutor.ZooKeeperRootKey, "/sparkone/test-overwrite")
  ...
  private def assertNoWorkDirectory(workspace: NioPath): Unit = {
    ...
    try {
      assertFalse(stream.iterator().asScala.exists(_.getFileName.toString.startsWith(".sparkone-overwrite-")))
    } finally {
      stream.close()
    }
  }
  ...
  private def simulateInterruptedPublish(spark: SparkSession, workspace: NioPath): Unit = {
    ...
    val hash = MessageDigest.getInstance("SHA-256")
      ...
      .mkString
      .take(24)
    val work = new HadoopPath(qualifiedTarget.getParent, s".sparkone-overwrite-$hash")

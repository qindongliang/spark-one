...
object ManagedHdfsOverwriteProtocol {
  private val Command = "QUERYONE MANAGED_HDFS_OVERWRITE"
...
private[hdfs] final case class ManagedHdfsCommandFields(
    tenant: String,
    table: String,
    format: String,
    relativePath: String,
...
private[hdfs] object ManagedHdfsCommandCodec {
  ...
  private def decode(token: String, operation: String): ManagedHdfsCommandFields = {
    val input = try {
      ...
      case e: IllegalArgumentException =>
        throw new IllegalArgumentException(s"Invalid QueryOne managed HDFS $operation command", e)
    }
    try {
      val version = input.readInt()
      if (version != Version) {
        throw new IllegalArgumentException(s"Unsupported QueryOne managed HDFS $operation command version: $version")
      }
      ...
      if (optionCount < 0 || optionCount > 100) {
        throw new IllegalArgumentException(s"Invalid QueryOne managed HDFS $operation option count")
      }
      val options = (0 until optionCount).map(_ => input.readUTF() -> input.readUTF()).toMap
      if (input.available() != 0) {
        throw new IllegalArgumentException(s"Invalid trailing data in QueryOne managed HDFS $operation command")
      }

...
private[hdfs] object ManagedHdfsOverwriteExecutor {
  val WorkspaceRootKey = ManagedHdfsWorkspacePolicy.WorkspaceRootKey
  val ZooKeeperConnectKey = "spark.queryone.overwrite.zk.connect"
  val ZooKeeperRootKey = "spark.queryone.overwrite.zk.root"
  val ZooKeeperSessionTimeoutMsKey = "spark.queryone.overwrite.zk.sessionTimeoutMs"
  val ZooKeeperConnectionTimeoutMsKey = "spark.queryone.overwrite.zk.connectionTimeoutMs"

  private val DefaultZooKeeperRoot = "/queryone/overwrite"
  ...
  private def resolvePaths(
    ...
    val workPath = new Path(parent, s".queryone-overwrite-${sha256(finalPath.toString).take(24)}")
    OverwritePaths(
      fs,
      finalPath,
      workPath,
  ...
  private[hdfs] def targetLockKey(
      tenant: String,
      relativePath: String,
  ...
  private[hdfs] def lockData(operationId: String, qualifiedTarget: String): String =
...
private[hdfs] final case class ZooKeeperTargetLock(
    connect: String,
    root: String,
    key: String,
    data: String,

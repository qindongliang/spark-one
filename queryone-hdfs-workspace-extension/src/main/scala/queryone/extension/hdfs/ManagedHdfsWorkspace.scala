...
object ManagedHdfsWorkspacePolicy {
  val WorkspaceRootKey = "spark.queryone.overwrite.workspaceRoot"
  ...
  private val InternalWorkPrefix = ".queryone-overwrite-"
  private val ManagedLoadWorkspaceOwnerTag =
    TreeNodeTag[String]("queryone.managedHdfsLoad.workspaceOwner")
  private val ManagedLoadReadWorkspaceOwnerLocalProperty =
    "queryone.managedHdfsLoad.internalRead.workspaceOwner"
  private val ManagedLoadReadExpectedPathLocalProperty =
    "queryone.managedHdfsLoad.internalRead.expectedPath"
  private val ManagedOverwriteWriteLocalProperty =
    "queryone.managedHdfsOverwrite.internalWrite"
  ...
  private[hdfs] def validateRequest(
      workspaceOwner: String,
      table: String,
      format: String,
      relativePath: String,
  ...
  private[hdfs] def resolveTarget(
      spark: SparkSession,
      tenant: String,
...
private[hdfs] final case class ManagedHdfsTarget(fs: FileSystem, finalPath: Path)

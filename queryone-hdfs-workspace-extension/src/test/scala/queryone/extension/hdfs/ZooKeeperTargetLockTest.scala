...
final class ZooKeeperTargetLockTest {
  ...
  def rejectsSecondOwnerAndAllowsNextOwnerAfterRelease(): Unit = {
    ...
    try {
      val first = lock(
        server,
        "operationId=first-operation\ntarget=hdfs://nameservice1/public/queryone/user/alice/result")
        ...
        try {
          ...
          case e: IllegalStateException =>
            ...
            assertTrue(e.getMessage.contains(
              "lockPath=/queryone/test-overwrite/alice/reports~daily--target-key"))
            assertTrue(e.getMessage.contains("operationId=first-operation"))
            assertTrue(e.getMessage.contains("target=hdfs://nameservice1/public/queryone/user/alice/result"))
        }
      } finally {
        first.close()
      }
  ...
  def buildsReadableTenantLockKeyAndCompactValue(): Unit = {
    val key = ManagedHdfsOverwriteExecutor.targetLockKey(
      ...
      qualifiedTarget = "hdfs://nameservice1/public/queryone/user/alice/reports/daily result")
    ...
    assertEquals(
      "operationId=operation-1\ntarget=hdfs://nameservice1/public/queryone/user/alice/reports/daily result",
      data)

...
import ai.queryone.extension.overwrite.{ManagedHdfsOverwriteProtocol, ManagedHdfsOverwriteRequest, ManagedHdfsWorkspacePolicy}
...
private[queryone] object CatalogWriteSqlRenderer {
  def render(
      plan: WritePlan,
      sourceColumns: Seq[String],
    ...
    QueryOneSqlRender.renderInsertTable(
      plan.mode.name,
      plan.target.identifier,
      plan.sourceTable,
...
private[sql] object WriteSqlRenderer {
  ...
  def render(plan: WritePlan): String = plan.executionType match {
    case CatalogSql =>
      QueryOneSqlRender.renderQueryOneAction(
        "SAVE CATALOG",

...
final class QueryOneCompilerTest {
  ...
  def compilesBasicPipeline(): Unit = {
    ...
    val load = queryone.extension.hdfs.ManagedHdfsLoadProtocol.parse(statements.head.sql)
  ...
  def compilesExcelLoadWithProviderAlias(): Unit = {
    ...
    val request = queryone.extension.hdfs.ManagedHdfsLoadProtocol.parse(statement.sql)
  ...
  def compilesManagedHdfsOverwriteAndRejectsExternalOverwrite(): Unit = {
    ...
    val request = queryone.extension.hdfs.ManagedHdfsOverwriteProtocol.parse(managed.sql)
    assertTrue(request.isDefined)
  ...
  def rejectsManagedHdfsInternalCommandSubmittedAsNativeSql(): Unit = {
    val commands = Seq(
      queryone.extension.hdfs.ManagedHdfsOverwriteProtocol.render(
        queryone.extension.hdfs.ManagedHdfsOverwriteRequest(
          "bob", "users", "parquet", "reports/daily", Map.empty)),
      queryone.extension.hdfs.ManagedHdfsLoadProtocol.render(
        queryone.extension.hdfs.ManagedHdfsLoadRequest(
          "bob", "users", "parquet", "reports/daily", Map.empty)))
  ...
  def doesNotSplitSemicolonsInsideBackticks(): Unit = {
    ...
    val request = queryone.extension.hdfs.ManagedHdfsLoadProtocol.parse(sql)

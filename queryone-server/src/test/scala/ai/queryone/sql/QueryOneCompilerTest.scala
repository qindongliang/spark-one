...
final class QueryOneCompilerTest {
  private val compiler = new QueryOneCompiler()
  ...
  def compilesBasicPipeline(): Unit = {
    ...
    val load = ai.queryone.extension.overwrite.ManagedHdfsLoadProtocol.parse(statements.head.sql)
  ...
  def compilesInlineQueryAssertAsViolationQuery(): Unit = {
    ...
    assertEquals(
      """SELECT * FROM (select dt, count(*) as row_count
        |  from orders
        |  group by dt) queryone_assert_input WHERE NOT COALESCE((row_count > 0), FALSE)""".stripMargin,
      statement.sql)
    assertEquals(
  ...
  def generatedAssertSqlIsAcceptedBySparkSqlParser(): Unit = {
    val validatingCompiler = new QueryOneCompiler(new SparkSqlValidator)
  ...
  def compilesSetAsSelectAsRuntimeSqlVariable(): Unit = {
    ...
    assertEquals("SELECT 'SET' AS queryone_action, 'start_date' AS queryone_target", statement.sql)
  ...
  @Test
  def compilesStaticJdbcPartitionLoadWithoutRenderingCredentials(): Unit = {
    val statement = compiler.compile(
      """load jdbc.`mysql_static.analytics.users`
        |options partitionColumn = "id" and numPartitions = "4"
        |as users;
        |""".stripMargin).head

    assertTrue(statement.sql.startsWith(
      "CREATE OR REPLACE TEMPORARY VIEW users USING queryone_mysql OPTIONS"))
    assertTrue(statement.sql.contains("catalog 'mysql_static'"))
    assertTrue(statement.sql.contains("dbtable 'analytics.users'"))
    assertEquals(Some(LoadTargetType.Provider), statement.load.map(_.targetType))
    assertEquals(Some("mysql_static.analytics.users"), statement.load.map(_.path))
  ...
  @Test
  def givesHelpfulMessageForJdbcLoadMissingTargetAlias(): Unit = {
    val validatingCompiler = new QueryOneCompiler(new SparkSqlValidator)

    try {
      validatingCompiler.compile("load jdbc.`Dworks.queryone_city_result`;")
      fail("Expected CompileException")
    } catch {
      case e: CompileException =>
        assertTrue(e.getMessage, e.getMessage.contains("QueryOne LOAD requires a target temp view"))
        assertTrue(e.getMessage, e.getMessage.contains("load jdbc.`Dworks.queryone_city_result` as queryone_city_result"))
        assertTrue(e.getMessage, e.getMessage.contains("Add `as queryone_city_result`"))
  ...
  @Test
  def compilesStaticJdbcLoadWhereForPartitionedRead(): Unit = {
      val statement = compiler.compile(
        """load jdbc.`mysql_static.analytics.big_orders`
          |where "biz_date = '2026-06-10' and status = 'PAID'"
      ...
      assertTrue(statement.sql.startsWith(
        "CREATE OR REPLACE TEMPORARY VIEW big_orders_paid USING queryone_mysql OPTIONS"))
      assertEquals(Some("analytics.big_orders"), statement.load.flatMap(_.options.get("dbtable")))
      assertTrue(statement.load.exists(_.options.contains("whereClauseBase64")))
      assertEquals(Some("id"), statement.load.flatMap(_.options.get("partitionColumn")))
  ...
  @Test
  def compilesStaticJdbcLoadWhereWithoutPartitionOptions(): Unit = {
      val statement = compiler.compile(
        """load jdbc.`mysql_static.analytics.queryone_mysql_orders_demo`
          |where "biz_date = '2026-06-10' and status = 'PAID'"
          |as orders_paid_0610;
          |""".stripMargin).head

      assertEquals(
        "CREATE OR REPLACE TEMPORARY VIEW orders_paid_0610 AS SELECT * FROM " +
          "mysql_static.analytics.queryone_mysql_orders_demo WHERE biz_date = '2026-06-10' and status = 'PAID'",
        statement.sql)
      assertFalse(statement.load.exists(_.options.contains("partitionColumn")))
  }

  @Test
  def compilesStaticJdbcLoadPartitionOptionsWithoutWhere(): Unit = {
      val statement = compiler.compile(
        """load jdbc.`mysql_static.analytics.queryone_mysql_orders_demo`
          |options partitionColumn="id"
          |and lowerBound="1"
      ...
      assertEquals(Some("analytics.queryone_mysql_orders_demo"), statement.load.flatMap(_.options.get("dbtable")))
      assertEquals(Some("id"), statement.load.flatMap(_.options.get("partitionColumn")))
      assertEquals(Some("1"), statement.load.flatMap(_.options.get("lowerBound")))
  ...
  @Test
  def compilesOdepJdbcLoadPartitionColumnOnlyForRuntimeAutoBounds(): Unit = {
      val statement = compiler.compile(
        """load jdbc.`analytics.big_orders`
          |where "biz_date = '2026-06-10' and status = 'PAID'"
          |options partitionColumn="id"
          |as big_orders_paid;
          |""".stripMargin).head

      assertEquals(Some("big_orders"), statement.load.flatMap(_.options.get("dbtable")))
      assertEquals(Some("analytics"), statement.load.flatMap(_.options.get("alias")))
      assertEquals(Some("id"), statement.load.flatMap(_.options.get("partitionColumn")))
      ...
      assertEquals(Some("10"), statement.load.flatMap(_.options.get("numPartitions")))
      assertEquals(Some("10000"), statement.load.flatMap(_.options.get("fetchsize")))
  }
  ...
  @Test
  def rejectsJdbcLoadQueryOptionBecauseDbtableIsManagedByQueryOne(): Unit = {
      try {
        compiler.compile(
          """load jdbc.`analytics.big_orders`
  ...
  def compilesExcelLoadWithProviderAlias(): Unit = {
    ...
    val request = ai.queryone.extension.overwrite.ManagedHdfsLoadProtocol.parse(statement.sql)
  ...
  def rejectsUnsafeManagedHdfsLoadPathsAndOptions(): Unit = {
    Seq(
      ...
      "../alice/result",
      "reports/../result",
      "reports/.queryone-overwrite-target/staging").foreach { path =>
  ...
  def compilesManagedHdfsOverwriteAndRejectsExternalOverwrite(): Unit = {
    ...
    val request = ai.queryone.extension.overwrite.ManagedHdfsOverwriteProtocol.parse(managed.sql)
    assertTrue(request.isDefined)
  ...
  def rejectsManagedHdfsInternalCommandSubmittedAsNativeSql(): Unit = {
    val commands = Seq(
      ai.queryone.extension.overwrite.ManagedHdfsOverwriteProtocol.render(
        ai.queryone.extension.overwrite.ManagedHdfsOverwriteRequest(
          "bob", "users", "parquet", "reports/daily", Map.empty)),
      ai.queryone.extension.overwrite.ManagedHdfsLoadProtocol.render(
        ai.queryone.extension.overwrite.ManagedHdfsLoadRequest(
          "bob", "users", "parquet", "reports/daily", Map.empty)))
  ...
  def compilesSaveAppendToHiveTable(): Unit = {
    ...
    assertEquals(
      "SELECT 'SAVE CATALOG' AS queryone_action, 'users TO spark_catalog.default.users' AS queryone_target",
      statement.sql)
  ...
  @Test
  def compilesStaticJdbcSaveAsCatalogSql(): Unit = {
    val statement = compiler.compile(
      "save append users as jdbc.`mysql_static.analytics.user_stats`;").head

    assertEquals(
      "SELECT 'SAVE CATALOG' AS queryone_action, 'users TO mysql_static.analytics.user_stats' AS queryone_target",
      statement.sql)
    assertEquals(Some(WriteMode.Append), statement.writePlan.map(_.mode))
    assertEquals(Some(WriteTargetKind.JdbcCatalog), statement.writePlan.map(_.target.kind))
    assertEquals(Some("mysql_static.analytics.user_stats"), statement.writePlan.map(_.target.identifier))
  }

  @Test
  def compilesJdbcLoadAsOdepRoutingCatalogReadButRejectsOdepSave(): Unit = {
  ...
  @Test
  def preservesDirectThreePartSelectForOdepAndStaticCatalogs(): Unit = {
    Seq(
      "select * from jdbc.search_prod.users",
      "select * from mysql_static.analytics.users").foreach { sql =>
      assertEquals(sql, compiler.compile(sql + ";").head.sql)
    }
  }
  ...
  def rejectsJdbcConnectionOptionsAndNonAliasTablePaths(): Unit = {
    ...
    Seq(
      ...
      "load jdbc.`jdbc_prod.search_prod.users` as users;").foreach { sql =>
      val error = tryCompile(sql)
      assertTrue(error.getMessage.contains("LOAD jdbc"))
    }

    val partitioned = compiler.compile(
      "load jdbc.`search_prod.users` options partitionColumn='id' as users;").head
    assertTrue(partitioned.sql.startsWith(
      "CREATE OR REPLACE TEMPORARY VIEW users USING queryone_mysql OPTIONS"))
    assertTrue(partitioned.sql.contains("catalog 'jdbc'"))
    assertTrue(partitioned.sql.contains("alias 'search_prod'"))
    assertTrue(partitioned.sql.contains("dbtable 'users'"))
    assertTrue(partitioned.sql.contains("partitionColumn 'id'"))
    assertTrue(partitioned.sql.contains("numPartitions '10'"))
    assertTrue(partitioned.sql.contains("fetchsize '10000'"))
  }
  ...
  def compilesSaveAppendToDorisCatalogTable(): Unit = {
    ...
    assertEquals(
      "SELECT 'SAVE CATALOG' AS queryone_action, 'users TO doris.dataagent.user_stats' AS queryone_target",
      statement.sql)
    assertEquals(Some(WriteMode.Append), statement.writePlan.map(_.mode))
  ...
  def doesNotSplitSemicolonsInsideBackticks(): Unit = {
    ...
    val request = ai.queryone.extension.overwrite.ManagedHdfsLoadProtocol.parse(sql)

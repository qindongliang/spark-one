...
import ai.queryone.extension.overwrite.{ManagedHdfsWorkspacePolicy, QueryOneHdfsOverwriteExtensions}
import ai.queryone.identity.TenantContext
import ai.queryone.kyuubi.odep.authz.LocalExecutionSubject
...
final class QueryOneRuntimePreviewTest {
  @Test
  def scopesLocalSubjectForRunAndPreview(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-subject-")
    var observedSubjects = Seq.empty[Option[String]]
    val spark = SparkSession.builder()
      .appName("QueryOne LocalSubjectTest")
      .master("local[1]")
  ...
  def localRuntimeRegistersOdepCapabilitiesWithoutEagerOdepAccess(): Unit = {
    withSystemProperties(Map("spark.master" -> "local[1]")) {
      val runtime = QueryOneRuntime.local()
      try {
        val spark = SparkSession.getActiveSession.get
        assertEquals(
          "ai.queryone.kyuubi.odep.catalog.OdepRoutingCatalog",
  ...
  def loadStatementReturnsPreviewRowsFromRegisteredView(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-preview-")
  ...
  def multipleSuccessfulLoadsOnlyReturnLastSchemaResult(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-preview-multiple-loads-")
    val spark = localSpark(root)
    try {
      import spark.implicits._
  ...
  def runtimeClampsExplicitLimitToPreviewMaxRows(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-preview-clamp-")
    val spark = localSpark(root)
    try {
  ...
  def runSubstitutesLiteralSetVariablesInLaterStatements(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-set-literal-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def runEvaluatesSqlSetVariablesBeforeLaterStatements(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-set-sql-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def assertPassesAndContinuesWithLaterStatements(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-assert-pass-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def inlineQueryAssertPassesAndContinuesWithLaterStatements(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-inline-assert-pass-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def inlineQueryAssertCanStopLaterStatementsWithoutFailingTheRun(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-inline-assert-fail-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def assertFailureReturnsLimitedViolationsAndStopsLaterStatements(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-assert-fail-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def assertTreatsNullPredicateAsFailure(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-assert-null-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def assertTreatsEmptyResultTableAsPassed(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-assert-empty-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  def assertReportsQueryExecutionErrorsSeparatelyFromBusinessFailures(): Unit = {
    val root = Files.createTempDirectory("queryone-runtime-assert-error-")
    val spark = localSpark(root)
    try {
      val runtime = new QueryOneRuntime(spark)
      val result = runtime.run(
  ...
  private def localSpark(root: Path): SparkSession = {
    SparkSession.builder()
      .appName("QueryOne RuntimePreviewTest")
      .master("local[1]")
      .config("spark.ui.enabled", "false")
      .config("spark.driver.host", "127.0.0.1")
      .config("spark.driver.bindAddress", "127.0.0.1")

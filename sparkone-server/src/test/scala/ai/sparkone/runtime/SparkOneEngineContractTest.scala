 ...
 */
abstract class SparkOneEngineContract {
  private val tenant = TenantContext.development("contract-user")

  protected def withEngine(body: SparkOneEngine => Unit): Unit
...
final class LocalSparkEngineContractTest extends SparkOneEngineContract {
  override protected def withEngine(body: SparkOneEngine => Unit): Unit = {
    val root = Files.createTempDirectory("sparkone-local-engine-contract-")
    SparkSession.clearActiveSession()
    SparkSession.clearDefaultSession()
    val spark = SparkSession.builder()
      .appName("SparkOne Local Engine Contract Test")
      .master("local[1]")
...
final class KyuubiSparkEngineContractTest extends SparkOneEngineContract {
  override protected def withEngine(body: SparkOneEngine => Unit): Unit = {

...
final class DataSourceResolver(
  ...
  private def jdbcCatalogTarget(path: String, statementType: String): JdbcCatalogTarget = {
    ...
    parts.length match {
      case 2 =>
        val alias = SparkOneSqlRender.requireIdentifier(parts(0), s"$statementType jdbc alias")
        val table = SparkOneSqlRender.requireIdentifier(parts(1), s"$statementType jdbc table")
        JdbcCatalogTarget(
          catalog = "jdbc",
          namespace = alias,
          table = table,
          odepRouted = true)
      case 3 =>
        val catalog = SparkOneSqlRender.requireIdentifier(parts(0), s"$statementType jdbc static catalog")
        ...
        val database = SparkOneSqlRender.requireIdentifier(parts(1), s"$statementType jdbc database")
        val table = SparkOneSqlRender.requireIdentifier(parts(2), s"$statementType jdbc table")
        JdbcCatalogTarget(
          catalog = catalog,
          namespace = database,
          table = table,
  ...
  private def normalizeJdbcLoadOptions(options: Seq[(String, String)]): Seq[(String, String)] = {
    ...
    optionMap.get("partitionColumn").foreach { value =>
      SparkOneSqlRender.requireIdentifier(value, "JDBC partitionColumn")
    }
